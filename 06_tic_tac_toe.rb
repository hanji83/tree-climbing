  ...
  def move(game, mark)
    ...
    node.children.each do |child|
      if child.winning_node?(mark)
        p "Making a winning move"
        return child.prev_move_pos
      elsif child.losing_node?(mark)
        losing_nodes << child.prev_move_pos
      else
        non_losing_nodes << child.prev_move_pos
      end #if
    end #each

    if !non_losing_nodes.empty?
      p "Making a non-losing move"
      return non_losing_nodes.first
    end

    p "Making a losing move"
    return losing_nodes.first
  end #move
end #SuperComputerPlayer

if __FILE__ == $PROGRAM_NAME
  hp = HumanPlayer.new("AAAC")
  scp = SuperComputerPlayer.new
  cp = ComputerPlayer.new

  TicTacToe.new(hp, scp).run
end
